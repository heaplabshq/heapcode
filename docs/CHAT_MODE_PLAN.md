...
| 2026-09-09 | Heap Chat never writes to the folder; what it produces goes to **artifacts**, and the person saves one into their files themselves | These are someone's documents — a tenancy agreement, a scan, six years of exports — and an assistant that edits them in place is one bad edit from destroying something with no other copy. Heap Code writes to a repo because a repo has git; a documents folder has nothing. Artifacts live under the state dir, outside the folder, and `saveArtifact` is the only path in — an explicit act, on their terms |
| 2026-09-09 | No shadow-git, no checkpoints, no diffs and no permission prompts in Heap Chat | All four exist to make an overwrite survivable. Nothing here overwrites, so there is nothing to snapshot, nothing to diff and nothing to revert — and a permission prompt for a read is noise that teaches people to click through prompts |
| 2026-09-09 | The panel is Heap Chat's own (`chat-ui/components/Panel.tsx`), not `web-ui`'s `Panel` | web-ui's is built around changes, diffs, checkpoints, a terminal and a repo map, none of which this product has. It uses the same classes and reuses `Preview` wholesale, so it looks identical; the four tabs are Files, Made, Sources and Index — the folder, what was made from it, and where the last answer came from |
| 2026-09-09 | The grounding badge opens the panel instead of expanding in place | Expanding pushed a table of snippets between the answer and the composer and moved the conversation around. The evidence belongs beside the transcript, where the file list and preview already are |
| 2026-09-09 | MCP connectors are in Heap Chat, from the same config Heap Code reads | Same reasoning as connections and the role table: one config, one place to register a server. Its tools are offered on top of the static roster and dispatched by the same host |
