...
| 2026-08-27 | The setting and the grant are shown separately | An optional permission cannot be granted at install, so a fresh install is on-but-not-granted. A switch reading "on" while nothing had actually changed is worse than one reading "off" |
| 2026-08-27 | `permissions.request` is called with nothing awaited before it | The gesture dies at the first await. Checking whether the permission was already held first — the obvious way to avoid a redundant prompt — meant the prompt never appeared, the switch sprang back, and nothing said why. It typechecks perfectly |
