...
| 2026-08-27 | **Revised:** handles name elements (`WeakRef` / backend node id), not positions, and no longer expire | Expiry was right *for positions* — `[4]` means something else after a re-render — and it cost most of every run, since the agent had to re-read between each action to learn numbers it had just been given. Identity gives the same safety more precisely: the reference dies when that node is replaced, survives when the page merely reorders, and expiry only ever knew that *something* had changed. Origin change is checked separately, which is the case a counter cannot tell from a harmless re-render |
...
| 2026-08-27 | Screenshots stream to the user every read; the model gets one only when it calls `screenshot` | A picture is 100–500KB and is re-sent on every later request once in the transcript — attaching one automatically is how a browser agent becomes slow and expensive by the fifth step. As a tool the model reaches for when text has failed, the cost is paid once and for a reason |
