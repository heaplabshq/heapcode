...
import { htmlToText, MAX_FETCH_CHARS } from './webText.js';

// Re-exported so the package's own API does not move: these used to live here,
// and existing imports (CLI, extension, tests) keep working through the
// package barrel. The browser-safe barrel exports them from `webText` directly.
export { decodeHtmlEntities, htmlToText, MAX_FETCH_CHARS } from './webText.js';
