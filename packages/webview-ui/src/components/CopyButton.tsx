 ...
 *
 * An icon rather than the word, and the outcome is an icon too, so the button
 * does not change width under the cursor that just clicked it. The accessible
 * name carries the meaning — a screen reader and a tooltip both read it.
 *
 * Deliberately a local copy of `@heapcode/web-ui`'s rather than an import:
 * this package depends on core alone, and pulling the browser UI in for one
 * button would bring its stylesheet and component tree into the webview
 * bundle. Only the class names are shared.
 */

/** The same stroke icons the other surfaces use. */
const S = {
  width: 14,
  height: 14,
  viewBox: '0 0 24 24',
...
export function CopyButton({ text, label = 'Copy' }: { text: string; label?: string }): JSX.Element {
  ...
  return (
    <button
      ...
      onClick={() => void copy()}
      aria-label={name}
      title={state === 'failed' ? 'Could not copy — the editor refused clipboard access' : name}
    >
